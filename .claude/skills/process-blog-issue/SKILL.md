...
- slug は英数字とアンダースコアのみを使用する
- **slug の自動補正**: issue から取得した slug にハイフン（`-`）が含まれていた場合は、アンダースコア（`_`）に置換してから使用する（例: `react-hooks` → `react_hooks`）。frontmatter の `slug` と記事ディレクトリ名の `<slug>` 部分の両方に補正後の値を使うこと
- タグが空の場合は空配列 `[]` を設定
- 感想が長い場合は適切に段落分けする

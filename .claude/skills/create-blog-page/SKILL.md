---
name: create-blog-page
description: 読んだ記事やスライドの感想ブログ記事を作成します
---

# Create Blog Page

読んだ記事やスライドのリンクと感想を書くブログ記事を作成します。

## 実行手順

### 1. ユーザーから情報を収集

以下の情報をユーザーに直接質問して収集してください：

- **ブログ記事のタイトル**: ブログページに表示されるタイトル
- **slug**: URLに使用される識別子（英数字とアンダースコアのみ、ハイフンは使わない）
- **タグ**: 記事のタグ（カンマ区切り、任意）
- **日付**: ファイルパスに使用する日付（デフォルト: 当日）
- **資料のタイトル**: 読んだ記事/スライドのタイトル（任意。省略時はURLがそのまま表示されます）
- **資料のURL**: 資料へのリンク
- **感想・学び**: 感想や学んだことの内容

### 2. ブログ記事の作成

以下の形式でファイルを作成します：

- **ファイルパス**: `blog/YYYY/MM-DD-slug/index.md`
  - `YYYY` は年（例: 2026）
  - `MM-DD` は月日（例: 01-30）
  - `slug` はユーザーが指定した値を使用（単語区切りはアンダースコア）
- **例**: `blog/2026/01-30-react_hooks/index.md`

### 3. テンプレート

`blog/_template.md` を参考に、以下の形式で作成：

```markdown
---
slug: /slug_name
title: ブログ記事のタイトル
authors: yumechi
tags: [tag1, tag2]
---

資料の概要や導入文（1-2文）

<!-- truncate -->

## 資料リンク

- [資料タイトル](URL)
  ※資料タイトルが空の場合はURLをそのままリンクテキストとして使用

## 感想・学び

感想や学びの内容をここに記載。
```

### 4. 作成後の確認

ファイル作成後、ユーザーに以下を確認：
- 作成したファイルのパス
- 内容のプレビュー
- 必要に応じて修正するか確認

## 注意事項

- slug は英数字とアンダースコアのみを使用（ハイフンは使わない）
- **slug の自動補正**: ユーザーから受け取った slug にハイフン（`-`）が含まれていた場合は、警告を出したうえでアンダースコア（`_`）に置換してから使用する（例: `react-hooks` → `react_hooks`）。同様にディレクトリ名の `<slug>` 部分も補正後の値を使う
- タグは既存のブログ記事で使われているタグを参考にする
- 感想が長い場合は適切に段落分けする
- 日付を指定しない場合は当日の日付を使用する
- **チルダ（`~`）のエスケープ**: タイトルや本文にASCIIチルダ `~` が含まれる場合、markdownの打消し線として解釈されるのを防ぐため以下の対応を行う：
  - **frontmatterの `title`**: ダブルクォートで囲む（例: `title: "タイトル~サブタイトル~"`）
  - **markdown本文**（導入文、リンクテキスト、感想など）: `~` を `\~` にエスケープする
  - 全角チルダ `〜`（U+301C）は対象外（markdownで打消し線にならない）
